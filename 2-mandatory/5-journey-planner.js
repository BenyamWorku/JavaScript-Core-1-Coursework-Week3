...
*/
function getTransportModes(locationAndMode) {
  // locationAndMode.shift();
  return locationAndMode.slice(1);
}
...
*/
function isAccessibleByTransportMode(transportModesArray, transportMode) {
  return transportModesArray.includes(transportMode);
}
...
/*
  Implement the function getLocationName that
   ...
   - Returns the name of the location
      e.g: "Tower Bridge"
*/
function getLocationName(locationAndMode) {
  return locationAndMode[0];
}
...
function journeyPlanner(locations, transportMode) {
  ...
  let placesYouCanAccess = [];
  for (elem of locations) {
    if (elem.includes(transportMode)) {
      placesYouCanAccess.push(elem[0]);
    }
  }
  return placesYouCanAccess;
}
